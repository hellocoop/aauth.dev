		...
		(async () => {
			...
			const program = new Program(gl, {
				...
				uniforms: {
					...
					uSpeed: { value: speed },
					uAnimType: { value: { rotate: 0, rotate3d: 1, hover: 2 }[animationType] ?? 1 },
					uMouse: { value: [0.5, 0.5] },
					uColorCount: { value: 0 },
					uDistort: { value: distort },
					uOffset: { value: [offset?.x || 0, offset?.y || 0] },
			...
			if (Array.isArray(colors) && colors.length > 0) {
				...
				gradientTex.width = capped.length;
...
<div bind:this={container} class="absolute inset-0 overflow-hidden pointer-events-none"></div>
